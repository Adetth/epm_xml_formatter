...
class XMLAnalyzer:
                ...
                for dimension in segment.findall("dimension"):
                    ...
                    for child in dimension:
                        if child.tag == "formula":
                            dim_items.append({"name": child.get("label", "Formula"), "func": child.get("label", "Formula"), "type": "FORMULA"})
                            ...
                            func_name = child.get("name", "Func")
                            dim_items.append({"name": raw_name, "func": f"{func_name}({raw_name})", "type": "FUNCTION"})
                        elif child.tag == "member":
                            if child.get("exclude") == "true": continue
                            dim_items.append({"name": child.get("name", ""), "func": child.get("name", ""), "type": "MEMBER"})
                    ...
                    if dim_items:
                        ...
                        for combo in combos:
                            for item in dim_items:
                                ...
                                new_c[f"_func_{dim_name}"] = item.get("func", item["name"])
    ...
    def add_location_dvr(self, row_loc, col_loc, style_id, hex_color, rule_name="Auto Format Rule"):
        ...
        for dvr in dvr_bucket.findall("dataValidationRule"):
            try:
                pos = int(dvr.get("position", "0"))
                if pos > max_pos: max_pos = pos
            except ValueError:
                pass
        new_pos = str(max_pos + 1)
        
        rule = ET.SubElement(dvr_bucket, "dataValidationRule", position=new_pos, name=rule_name, description="Automated layout styling rule generated by the EPM XML Multi-Tool. © Adetth Raju", enabled="true", customStyle="true", rowLocation=str(float(row_loc)), colLocation=str(float(col_loc)))
